...
const Variants = () => {
  ...
  const downloadCSV = () => {
    ...
    const rows = promoterdata.map((promoter) => [
      ...
      escapeCSVValue(promoter["GeneTPM"]), // CHANGE
      escapeCSVValue(`${promoter.Chr}:${promoter.TSS}`),
      escapeCSVValue(promoter["Transcript"]),
      // escapeCSVValue(promoter["Transcript_TPM"]),
      escapeCSVValue(promoter["TranscriptTPM"]), // CHANGE
      escapeCSVValue(promoter["Transcript_support_level"]), // ADD
      ...
      escapeCSVValue(promoter.Type), // ADD
      escapeCSVValue(promoter.Score), // ADD
      escapeCSVValue(promoter.Score_source), // ADD
  ...
  return (
    <div className="variant-page-container">
      ...
      {variantdata === null && (
        <h1 className="no-results-message">{`No results found for ${Id}`}</h1>
      )}
      {variantdata === null ? null : (
        <div className="table-wrapper">
          <h3>
            {Id} in {celltype} cell-type
          </h3>
                ...
                <tbody key={variant._id}>
                  <tr>
                    {variantdata && variantdata.length > 1 && (
                      <td>
                        <input
                          ...
                          name="selectVariant"
                          value={variant}
                          onChange={() => setShowallele(variant)}
                        />
                      </td>
                    )}
                    ...
                    <td className="narrow-column">
                      <a
                        href={`http://www.ncbi.nlm.nih.gov/snp/${variant.RSID}/`}
                        target="_blank"
                        className="variant-external-links"
                      >
                        {variant.RSID}
                      </a>
                    </td>
                    <td>
                      {`Chr${variant.Chr}:${variant.Start}:${variant.Ref}:`}
                      <span style={{ color: "red" }}>{variant.Alt}</span>
                    </td>
                    <td className="narrow-column">{variant.Region_Ensembl}</td>
                    <td className="narrow-column">
                      {variant.GeneName_ID_Ensembl}
                    </td>
                    ...
                    <td>
                      ...
                      {celltype === "hMSC"
                        ...
                        ? variant.OpenChromatin_OB
                        : celltype === "Osteocyte"
                        ? variant.OpenChromatin_OC
                        : celltype === "Myoblast" // ADD
                        ? variant.OpenChromatin_MB
                        : celltype === "Myotube" // ADD
                        ? variant.OpenChromatin_MT
                        : "NA"}
                    </td>
      ...
      {promoterdata && promoterdata.length > 0 && (
        <>
          ...
          <div className="table-wrapper">
            <h3>
              {Id}'s Hi-C interactions in {celltype} cell-type
            </h3>
            <table className="table">
              <thead>
                <tr>
                  ...
                  <th>TRANSCRIPT SUPPORT LEVEL</th> {/* ADD */}
