...
const Genes = () => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (res.ok) {
        ...
        if (result.gene[0]?.Gene_Name) {
          await waitForBedpe(
            `${result.gene[0].Gene_Name}_${celltype}.bedpe.txt`
          );
        }
        setBedpeReady(true);
      } else {
  ...
  const handleDiseasePageChange = (offset) => {
    setCurrentDiseaseItems(
      diseases.slice(offset, offset + diseaseItemsPerPage)
    );
  };

  // proximal Results pagination
  const handleProximalPageChange = (offset) => {
    setCurrentProximalItems(
      proximalRegion.slice(offset, offset + proximalItemsPerPage)
    );
  };

  // distal Results pagination
  const handleDistalPageChange = (offset) => {
    setCurrentDistalItems(
      distalRegion.slice(offset, offset + distalItemsPerPage)
    );
  };
  ...
  const fetchProximalRegion = async () => {
    ...
    const url = new URL(
      `${import.meta.env.VITE_EXPRESS_URL}/genes/${Id}/proximal_regulatory`
    );
    url.search = new URLSearchParams({ celltype: celltype }).toString();
  ...
  const fetchDistalRegion = async () => {
    ...
    const url = new URL(
      `${import.meta.env.VITE_EXPRESS_URL}/genes/${Id}/distal_regulatory`
    );
    url.search = new URLSearchParams({ celltype: celltype }).toString();
  ...
  return (
    <div>
        ...
        <div className="gene_card">
          ...
          <div className="gene_info">
            <div className="gene_info_row">
              ...
              <span>
                <a
                  className="gene_external_link"
                  href={`http://www.ensembl.org/Homo_sapiens/Gene/Summary?g=${genedata.ID}`}
                  target="_blank"
                >
                  {genedata.ID}
                </a>
            ...
            <div className="gene_info_row">
              ...
              <span>
                {genedata.Strand === "+" ? `Plus strand` : `Minus strand`}
              </span>
            </div>
            <div className="gene_info_row">
            ...
            <div className="gene_info_row">
              ...
              <a
                className="gene_external_link"
                href={`https://www.genecards.org/cgi-bin/carddisp.pl?gene=${genedata["Gene_Name"]}`}
                target="_blank"
              >
                {genedata["Gene_Name"]}
              </a>
      ...
      {genedata && celltype && Igvrange && bedpeReady ? (
        <div className="gap">
          <div>
      ...
      {activeTab === 1 && (
        <div>
          <button
            onClick={() => downloadCSV("codingRegion")}
                ...
                return (
                  <tbody key={index} className="disease-row-tbody">
                    <tr>
                      <td>
                        <a
                          href={`http://www.ncbi.nlm.nih.gov/snp/${region.RSID}/`}
                          target="_blank"
                          className="gene_external_link"
                        >
                          {region.RSID}
              ...
              <div className="items-per-page-options-container">
                ...
                <span>
                  ...
                  {currentItems.length > 0
                    ? `${codingRegion.indexOf(currentItems[0]) + 1} to ${
                        codingRegion.indexOf(
                          currentItems[currentItems.length - 1]
                        ) + 1
                      }`
                    : "0"}{" "}
                  of {codingRegion.length} Results
                </span>
              </div>
            )}
          </div>
      ...
      {activeTab === 2 && (
        <>
          <div className="table-wrapper">
            ...
            {loadingProximal ? (
              ...
              <table className="table">
                ...
                {proximalRegion && proximalRegion.length > 0 ? (
                  currentProximalItems.map((region) => {
                    return (
          ...
          <div className="pagination-container">
            {proximalRegion && (
              <Pagination
                key={proximalItemsPerPage}
                itemsPerPage={proximalItemsPerPage}
                items={proximalRegion}
                onPageChange={handleProximalPageChange}
              />
            )}
            {proximalRegion && (
              <div className="items-per-page-options-container">
                ...
                <span>
                  {" "}
                  Showing{" "}
                  {currentProximalItems.length > 0
                    ? `${
                        proximalRegion.indexOf(currentProximalItems[0]) + 1
                      } to ${
                        proximalRegion.indexOf(
                          currentProximalItems[currentProximalItems.length - 1]
                        ) + 1
                      }`
                    : "0"}{" "}
                  of {proximalRegion.length} Results
                </span>
              </div>
            )}
          </div>
            ...
            {distalRegion && (
              <div className="items-per-page-options-container">
                ...
                <span>
                  {" "}
                  Showing{" "}
                  {currentDistalItems.length > 0
                    ? `${distalRegion.indexOf(currentDistalItems[0]) + 1} to ${
                        distalRegion.indexOf(
                          currentDistalItems[currentDistalItems.length - 1]
                        ) + 1
                      }`
                    : "0"}{" "}
                  of {distalRegion.length} Results
                </span>
              </div>
            )}
          </div>
      ...
      <>
                  ...
                  <tbody key={disease._id} className="disease-row-tbody">
                    <tr>
                      ...
                      <td>
                        <a
                          className="gene_external_link"
                          href={`https://www.ncbi.nlm.nih.gov/snp/${disease["RSID"]}`}
                        >
                          {disease["RSID"]}
                        </a>
                      </td>
                      ...
                      <td>
                        <a
                          className="gene_external_link"
                          href={`http://pubmed.ncbi.nlm.nih.gov/${disease["Pubmed"]}/`}
                          target="_blank"
                        >
                          {disease["Pubmed"]}
                        </a>
                      ...
                      <td>
                        <a
                          className="gene_external_link"
                          href={`http://www.ebi.ac.uk/gwas/studies/${disease["STUDY_ACCESSION"]}`}
                          target="_blank"
                        >
        ...
        <div className="pagination-container">
          {diseases && (
            <Pagination
              key={diseaseItemsPerPage}
              itemsPerPage={diseaseItemsPerPage}
              items={diseases}
          ...
          {diseases && (
            <div className="items-per-page-options-container">
              ...
              <span>
                {" "}
                Showing{" "}
                {currentDiseaseItems.length > 0
                  ? `${diseases.indexOf(currentDiseaseItems[0]) + 1} to ${
                      diseases.indexOf(
                        currentDiseaseItems[currentDiseaseItems.length - 1]
                      ) + 1
                    }`
                  : "0"}{" "}
                of {diseases.length} Results
              </span>
            </div>
          )}
        </div>
