...
const getPromoterData = async (celltype, gene) => {
  ...
  if (celltype === "hMSC") {
    ...
    // ADD THIS
    genes = await Promoter_MB.find({ Gene: `${gene[0]["Gene_Name"]}` });
  } else if (celltype === "Myotube") {
    // ADD THIS
    genes = await Promoter_MT.find({ Gene: `${gene[0]["Gene_Name"]}` });
  }
  ...
  const bedpeFilePath = path.join(
    publicFolderPath,
    `${gene[0]["Gene_Name"]}_${celltype}.bedpe.txt`
  );
...
const generateBedpeFile = async (genes, filePath) => {
  if (fs.existsSync(filePath)) {
    ...
    console.log(
      `BEDPE file already exists at ${filePath}, reading the file...`
    );
    return existingFile;
  } else {
    ...
    const bedpeString = bedpeDataArray
      .map(
        (item) =>
          `${item.chr1}\t${item.start1}\t${item.end1}\t${item.chr2}\t${item.start2}\t${item.end2}\t${item.long}`
      )
      .join("\n");
...
router.get("/:id/proximal_regulatory", async (req, res) => {
  ...
  try {
    ...
    if (celltype === "hMSC") {
      ...
      // ADD THIS
      cellTypeField = "chromHMM_MB";
    } else if (celltype === "Myotube") {
      // ADD THIS
      cellTypeField = "chromHMM_MT";
    }
    ...
    if (variants.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No proximal regulatory data found" });
    }
  } catch (error) {
...
router.get("/:id/distal_regulatory", async (req, res) => {
  ...
  try {
    ...
    if (variants.length > 0) {
      ...
      return res
        .status(404)
        .json({ message: "No distal regulatory data found" });
    }
  } catch (error) {
    return res.status(500).send("Sever error");
  }
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const gene = await geneModel.find({ Gene_Name: id });
    const diseases = await Disease2variant.find({
      Reported_gene: { $regex: id, $options: "i" },
    }).sort({ "P-value": 1 });
    const codingRegion = await VariantModel.find({
      GeneName_ID_Ensembl: id,
      Region_Ensembl: "exonic",
    });
    if (gene.length > 0) {
